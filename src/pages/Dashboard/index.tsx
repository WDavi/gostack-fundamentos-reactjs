  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      ...
      const formattedTransactions = response.data.transactions.map(
        transaction => {
          const refactoredTransaction = transaction;

          if (transaction.type === 'income') {
            refactoredTransaction.formattedValue = formatValue(
              transaction.value,
            );
          } else {
            refactoredTransaction.formattedValue = `- ${formatValue(
          ...
          refactoredTransaction.formattedDate = formatDate(
            transaction.created_at,
          );

          return refactoredTransaction;
        },
      );
